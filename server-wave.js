...
async function sendPushNotification(adminEmail, title, body, data = {}) {
    try {
        ...
        if (!accessToken) {
            console.error('❌ FCM_ACCESS_TOKEN non configuré dans les variables d\'environnement');
            return false;
        }
    ...
    try {
        ...
        if (adminPush) {
            await sendPushNotification(
                adminPush.email,
                `🌊 Nouvelle demande Wave`,
                `Commande #${commande_id} de ${client?.name || 'Client'} - ${montantWave} FCFA`,
                { commandeId: commande_id.toString(), type: 'wave_demande' }
            );
        }
    ...
    try {
        ...
        if (status === 'success') {
            ...
            if (adminPush) {
                await sendPushNotification(
                    adminPush.email,
                    `✅ Paiement Wave validé #${commandeId}`,
                    `${clientName} - ${montant} FCFA`,
                    { commandeId: commandeId.toString(), type: 'wave_success' }
                );
            }
            ...
            if (adminPush) {
                await sendPushNotification(
                    adminPush.email,
                    `❌ Paiement Wave refusé #${commandeId}`,
                    `${clientName} - Motif : ${cause}`,
                    { commandeId: commandeId.toString(), type: 'wave_refused' }
                );
            }
...
app.post('/api/wave/remboursement', async (req, res) => {
    ...
    try {
        ...
        if (adminPush) {
            await sendPushNotification(
                adminPush.email,
                `💰 Remboursement effectué #${verification.commande_id}`,
                `${clientName} - ${montant} FCFA`,
                { commandeId: verification.commande_id.toString(), type: 'wave_remboursement' }
            );
        }
